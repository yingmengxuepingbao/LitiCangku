$(function(){
    var _edgeBox_service={
        ...
        //根据id获取设备采集终端信息
        getEdgeBoxById : function(locationCode, resolve, reject){
            $.ajax({
                type	:"get",
                url		: $.mpbGetHeaderPath()+"/pallet/queryByLocationCode/"+locationCode,
    ...
    var _addEquipment= {
        ...
        bindEvent: function () {
            ...
            $(document).on('click', '.return', function (){
                ...
                _this.data.isSaveBeforeDevice   = true ;
                window.location.href="wmsLocationStereoscopicList.html";
            })
        },
        //加载设备信息
        loadEquipmentInfo : function () {
            var _this = this;
            if(window.location.href.split("=")[0].indexOf("locationCode")==-1) {
                ...
                $(".breadcrumb .active").text("库位库存明细");
                _this.data.locationCode = window.location.href.split("=")[1];
                console.log(window.location.href.split("="));
                _edgeBox_service.getEdgeBoxById(_this.data.locationCode, function (res) {
                    var result=res;
                    if(result.code==0) {
        ...
        loadEquipmentUnusedList:function (list) {
            ...
            if(list!=[]){
                ...
                var str="";
                for(ele in list){
                    str+='<tr  data-id="'+list[ele].palletId+'">' +
                        ...
                        '<td  class="box">' + list[ele].boxBarcodeList + '</td>' +
