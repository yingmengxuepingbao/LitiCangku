...
$(function(){
     ...
     _task= {
        ...
        bindEvent: function () {
            ...
            //             $("#cancelFlagSpan").text(data.cancelFlag);
            ...
            // })
            //table 数量点击事件 显示列表
            $(document).on('click' ,'.a-cancelFlag', function (e) {
                e.stopPropagation();
                var _self=$(this);
                ...
                var cancelFlag  =_self.parent().parent().find('.cancelFlag').text();
                $("#orderIdLi").text(orderNo);
                $("#orderTypeLi").text(orderItem);
                $("#orderStatusLi").text(orderStatus);

                $("#cancelFlag").text(cancelFlag);
        ...
        loadTasksHtml : function(list) {
            ...
            for(ele in list){
                ...
                var iEdit ='<a onclick="editOneRecord(\''+list[ele].orderOutId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>';

                str+='<tr data-id="'+list[ele].orderOutId+'">' ;
                    str+=strCheck;
                    str+=
                        ...
                        '<td class="cancelFlag" >'+list[ele].cancelFlag+'</td>';
                        // if(list[ele].realAmount!="0") {
                        //     iDelete='';//实际数量为0可删除
                        // }
        ...
        success	: function (res) {
            ...
            if(result.code == 0){
                ...
                if(data != null){
                    ...
                    $("#cancelFlag").val(data.cancelFlag);
...
$("#saveButton").on("click",function () {
    ...
    var cancelFlag = $.trim($("#cancelFlag").val());
    //添加数据校验
    if(_commonFun.isBlank(goodsCode)){
        $.DialogByZ.Autofade({Content: "商品编码不能为空"});
        return false;
    }
    if(_commonFun.isBlank(cancelFlag)){
