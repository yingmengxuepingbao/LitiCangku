...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/orderCrossIn/list",
    ...
    _frontBasic = {
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].orderCrossInId+'" data-orderNo ="'+list[ele].orderNo+'">' +
                ...
                // tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].orderCrossInId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '<td>'+ '<a onclick="deleteOneRecord(\''+list[ele].orderCrossInId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a  onclick="editSelect(\''+list[ele].orderCrossInId+'\',\''+list[ele].orderNo+'\',\''+list[ele].orderStatus+'\')" > <i class="fa-table-line-look" title="查看"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                // tbody +=  '<a  onclick="distribution(\''+list[ele].orderCrossInId+'\',\''+list[ele].orderNo+'\')" > <i title="分配库存">分配</i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                // tbody +=  '<a  onclick="cancelDis(\''+list[ele].orderCrossInId+'\',\''+list[ele].orderNo+'\')" > <i title="取消分配">取消分配</i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
    ...
    $(document).on('click', '.a-equipment-look', function (e) {
        ...
        window.location.href="addOrderCrossIn.html?id="+id;
    })
    ...
    $("#saveButton").on("click",function () {
            var orderCrossInId = $.trim($("#orderCrossInId").val());
            ...
            //判断是新增还是修改
            if (!_commonFun.isNotBlank(orderCrossInId)){
                data.orderNo= "";
                //不存在新增
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/orderCrossIn/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
                ...
                //存在id修改
                data.orderCrossInId = orderCrossInId;
                $.ajax({
                    type	:"POST",
...
function editSelect(orderCrossInId,orderNo,orderStatus) {
    // var _self = $(this);
    //获取详情
    console.log(orderCrossInId);
    // var id = _self.parent().parent().data('orderNo');
    window.location.href="addOrderCrossIn.html?orderCrossInId="+orderCrossInId+"&orderNo="+orderNo+"&orderStatus="+orderStatus;
}


//编辑 type  0 表示 查看  1 表示 编辑  2表示 审核
function editOneRecord(orderCrossInId,type) {
    var id = orderCrossInId;
    $("#type").val(type);
    //供应商
    // _buz_service.getSupplyList();
    ...
    $.ajax({
        ...
        success	: function (res) {
            ...
            if(result.code == 0){
                ...
                if(data != null){
                    $("#orderCrossInId").val(data.orderCrossInId);
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/orderCrossIn/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {orderCrossInId:id},
                success	: function (res) {
                    var result=res;
                    if(result.code == 0){
                        _commonFun.successTips(result.data);
...
function distribution(orderCrossInId,orderNo) {
    $.DialogByZ.Confirm({
        Title: "操作提示",
        Content:"确定要分配此出库单库存?",
        FunL:function(){
...
function cancelDis(orderCrossInId,orderNo) {
    $.DialogByZ.Confirm({
        Title: "操作提示",
