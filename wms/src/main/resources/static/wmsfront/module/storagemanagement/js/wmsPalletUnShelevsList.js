...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/pallet/unshelves/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                palletCode : $.trim($('#palletCode').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody +=  '<td><a  onclick="editSelect(\''+list[ele].palletCode+'\')" > <i class="fa-table-line-look" title="查看"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>';
            }
    ...
    $("#exportExcel").on("click",function () {
        let palletCode = $("#palletCode").val();
        let goodsCode = $("#goodsCode").val();
        let batchNo = $("#batchNo").val();
        window.location.href=$.mpbGetHeaderPath()+"/excel/unshelves?palletCode="+palletCode
            +"&goodsCode="+goodsCode
            +"&batchNo="+batchNo;
    });
...
function editSelect(palletCode) {
    window.location.href="wmsBoxBarcodeList.html?palletCode="+palletCode;
}
