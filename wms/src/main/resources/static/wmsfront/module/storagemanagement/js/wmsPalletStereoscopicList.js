...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/pallet/stereoscopic/list/hz",
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                tbody+='<tr data-id="'+list[ele].storagePlaneId+'">' +
                    ...
                    '<td>'+list[ele].number+'</td>'+
                    '<td>'+list[ele].amount+'</td>';
                tbody +=  '<td><a  onclick="editSelect(\''+list[ele].goodsCode+','+list[ele].batchNo+'\')" > <i class="fa-table-line-look" title="查看"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>';
            }
    ...
    $("#exportExcel").on("click",function () {
        ...
        window.location.href=$.mpbGetHeaderPath()+"/excel/stereoscopic?goodsCode="+goodsCode+"&batchNo="+batchNo;
    });
...
function editSelect(goodsCode, batchNo) {
    window.location.href="wmsPalletInfoStereoscopicList.html?goodsCode="+goodsCode+","+batchNo;
}
