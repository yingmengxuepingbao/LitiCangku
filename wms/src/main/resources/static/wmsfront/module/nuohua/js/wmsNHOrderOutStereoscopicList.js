    ...
    _edgeBox= {
        ...
        bindEvent: function () {
            ...
            $(document).on('click', '#addEquipment', function () {
                ...
                window.location.href="addNHOrderOutStereoscopic.html";
            });
            //点击EdgeBox采集终端列表中的编辑
            $(document).on('click', '.a-equipment-edit', function (e) {
                ...
                alert("id"+ id)
                window.location.href="addOrderOutStereoscopic.html?id="+id;
            })
            //全选/全不选 checkbox
...
function editOneRecord(orderNo,type) {
    // alert(orderNo);
    if(type == 0){
        window.location.href="infoNHOrderOutStereoscopic.html?orderNo="+orderNo+','+type;
    }else{
