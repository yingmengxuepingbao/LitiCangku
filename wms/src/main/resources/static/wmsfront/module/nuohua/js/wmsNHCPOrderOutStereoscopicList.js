    ...
    _edgeBox= {
        ...
        bindEvent: function () {
            ...
            $(document).on('click', '#addEquipment', function () {
                ...
                window.location.href="addNHCPOrderOutStereoscopic.html";
            });
            //点击EdgeBox采集终端列表中的编辑
            $(document).on('click', '.a-equipment-edit', function (e) {
                ...
                alert("id"+ id)
                window.location.href="addNHCPOrderOutStereoscopic.html?id="+id;
            })
            //全选/全不选 checkbox
            ...
            $(document).on('click', '.taskStartList', function () {
                ...
                _box_service.taskStartListBoxInfo(monitorStr, function (res) {
                    ...
                    if(result.code==0) {
                        //已生成，调任务列表页面
                        window.location.href="wmsNHTaskExecutionLogList.html?userDefined2=10&&orderNo="+monitorStr+"&taskType=20";
                    }else {
                        // 生成失败
                        $.DialogByZ.Autofade({Content: result.message});
                        _this.loadEdgeBoxs();
...
function editSelect(orderNo) {
    window.location.href="addNHCPOrderOutStereoscopic.html?orderNo="+orderNo;
}
...
function editOneRecord(orderNo,type) {
    // alert(orderNo);
    if(type == 0){
        window.location.href="infoNHOrderOutStereoscopic.html?orderNo="+orderNo+','+type +',10';
    }else{
        window.location.href="addNHCPOrderOutStereoscopic.html?orderNo="+orderNo+','+type;
    }
}
//停用edgeBox
