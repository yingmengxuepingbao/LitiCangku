...
$(function(){
    _edgeBox_service={
        ...
        getEdgeBoxById : function(orderNo, resolve, reject){
            $.ajax({
                type	:"get",
                url		: $.mpbGetHeaderPath()+"/orderCrossInDetail/queryByOrderNo/"+orderNo,
     ...
     _addEquipment= {
        ...
        bindEvent: function () {
            ...
            $("#saveButton").on("click",function () {
                var orderCrossInDetailId = $.trim($("#orderCrossInDetailId").val());
                console.log("orderCrossInDetailId",orderCrossInDetailId);
                ...
                //判断是新增还是修改
                if (!_commonFun.isNotBlank(orderCrossInDetailId)){
                    data.orderCrossInId =  _this.data.orderCrossInId;
                    //不存在新增
                    $.ajax({
                        type	:"POST",
                        url		: $.mpbGetHeaderPath()+"/orderCrossInDetail/c",
                        headers:{'token':localStorage.getItem('token')},
                        data    : data,
                        success	: function (res) {
                            var result=res;
                    ...
                    //存在id修改
                    data.orderCrossInDetailId = orderCrossInDetailId;
                    $.ajax({
                        type	:"POST",
                        url		: $.mpbGetHeaderPath()+"/orderCrossInDetail/u",
                        headers:{'token':localStorage.getItem('token')},
                        data    : data,
                        success	: function (res) {
                            var result=res;
            ...
            $(document).on('click', '.return', function (){
                ...
                window.location.href="wmsOrderCrossInList.html";
            })
        },
        //加载设备信息
        ...
        loadEquipmentUnusedList:function (list) {
            ...
            if(list!=[]){
                ...
                for(ele in list){
                    ...
                    str+='<tr  data-id="'+list[ele].orderCrossInDetailId+'">' +
