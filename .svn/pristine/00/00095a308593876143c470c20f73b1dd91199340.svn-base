...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/pallet/list1",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            var info={
                locationCode : serachOb.split(",")[0],
                goodsCode: serachOb.split(",")[1],
                batchNo: serachOb.split(",")[2],
    ...
    $("#rtn").on("click",function () {
        window.location.href="wmsStoragePlaneList.html";
    });
...
function editSelect(locationCode, goodsCode, batchNo) {
    window.location.href="wmsBoxBarcodeList.html?locationCode="+locationCode+"," + goodsCode + ","+batchNo;
}
