...
$(function(){
    _buz_service={
        ...
        updateStatsTask : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/nuohua/report/stereoscopicStatusUpdate",
                headers:{'token':localStorage.getItem('token')},
                data    : data || {},
                success	: resolve,
                error	: reject
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            var taskStatus = req['taskStatus'];
            var taskId = $.trim($('#taskIdPoP').val());
            var userDefined3 = $.trim($('#userDefined3PoP').val());
            console.log("taskType = "+taskType +"taskStatus ="+taskStatus);
            var info;
            if (taskId == ""){
                info={
                    taskType: taskType,
...
function  lookRecord(taskId,goodsCode,batchNo){
    console.log("taskId : "+taskId,"goodsCode : "+goodsCode,"batchNo : "+batchNo);
    $("#taskId_h").val(taskId);
