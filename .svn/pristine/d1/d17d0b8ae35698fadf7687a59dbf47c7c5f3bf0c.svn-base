...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/customer/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                customerCode    : $.trim($('#customerCodePoP').val()),
                customerFullName          : $.trim($('#customerFullNamePoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                tbody+='<tr data-id="'+list[ele].customerId+'">' +
                    '<td>'+list[ele].customerCode+'</td>'+
                    '<td>'+list[ele].customerFullName+'</td>'+
                    '<td>'+list[ele].customerShortName+'</td>'+
                    '<td>'+list[ele].customerEngName+'</td>'+
                    '<td>'+list[ele].customerType+'</td>'+
                    '<td>'+list[ele].customerLevel+'</td>'+
                    '<td>'+list[ele].address+'</td>'+
                    '<td>'+list[ele].contacts+'</td>'+
                    '<td>'+list[ele].website+'</td>'+
                    '<td>'+list[ele].registrationDate+'</td>';
                tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].customerId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].customerId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
    ...
    $("#saveButton").on("click",function () {
            ...
            var customerFullName = $.trim($("#customerFullName").val());
            ...
            //添加数据校验
            if(_commonFun.isBlank(customerCode)){
                $.DialogByZ.Autofade({Content: "客户编码不能为空"});
                return false;
            }
            if(_commonFun.isBlank(customerFullName)){
            ...
            if (!_commonFun.isNotBlank(customerId)){
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/customer/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/customer/u",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
...
function editOneRecord(customerId,type) {
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/customer/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/customer/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {customerId:id},
                success	: function (res) {
                    var result=res;
