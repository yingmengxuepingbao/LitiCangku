...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/combineLog/list",
        ...
        getSupplyList : function(data, reject){
            $.ajax({
                type	:"get",
                url		: $.mpbGetHeaderPath()+"/dictItem/getDictType/combineLog_type",
                headers:{'token':localStorage.getItem('token')},
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            // }
            var info={
                combineTaskId    : $.trim($('#combineTaskIdPoP').val()),
                fromLocationCode    : $.trim($('#fromLocationCodePoP').val()),
                toLocationCode : $.trim($('#toLocationCodePoP').val()),
                fromPalletCode : $.trim($('#fromPalletCodePoP').val()),
                toPalletCode : $.trim($('#toPalletCodePoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].combineLogId+'">' +
                    // '<td>'+list[ele].combineTaskId+'</td>'+
                    '<td>'+list[ele].fromPalletCode+'</td>'+
                    '<td>'+list[ele].toPalletCode+'</td>'+
                    '<td>'+list[ele].fromLocationCode+'</td>'+
                    '<td>'+list[ele].toLocationCode+'</td>'+
                    '<td>'+list[ele].goodsCode+'</td>'+
                    '<td>'+list[ele].fromAmount+'</td>'+
    ...
    $("#saveButton").on("click",function () {
            ...
            var combineTaskId = $.trim($("#combineTaskId").val());
            ...
            //添加数据校验
            if(_commonFun.isBlank(combineTaskId)){
            ...
            if (!_commonFun.isNotBlank(customerId)){
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/combineLog/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/combineLog/u",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
...
function editOneRecord(customerId,type) {
    ...
    if(type == 0 ){
        ...
        $("form  input").prop("disabled",false);
        //路线编码默认不可编辑
        $("#combineTaskId").prop("disabled",true);
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/combineLog/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
            if(result.code == 0){
                ...
                if(data != null){
                    ...
                    $("#combineTaskId").val(data.combineTaskId);
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/combineLog/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {customerId:id},
                success	: function (res) {
                    var result=res;
