...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/callAgvLog/list",
        ...
        findAddress : function(data, reject){
            $.ajax({
                type: "post",
                url: $.mpbGetHeaderPath() + "/address/findAgvAddress",
                headers: {'token': localStorage.getItem('token')},
                data: {
                },
                async: false,
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                taskCode    : $.trim($('#taskCodePop').val()),
                reqFlag          : $.trim($('#taskStatusPoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].id+'">' +
                    '<td style="width: 5%">'+list[ele].taskCode+'</td>'+
                    '<td style="width: 10%">'+list[ele].reqCode+'</td>'+
                    '<td style="width: 10%">'+list[ele].interfaceName+'</td>'+
                    '<td style="width: 5%">'+list[ele].taskType+'</td>'+
                    '<td style="width: 15%">'+list[ele].positionCode+'</td>'+
                    '<td style="width: 5%">'+reqFlagText+'</td>'+
                    '<td style="width: 15%">'+list[ele].respData+'</td>'+
                    '<td style="width: 4%">'+list[ele].userDefined1+'</td>'+
                    '<td>'+list[ele].userDefined2+'</td>'+
                    '<td style="width: 8%">'+list[ele].sendTime+'</td>'+
                    '<td style="width: 8%">'+list[ele].respTime+'</td>';
                tbody += '<td>'+'<a onclick="startOneRecord(\''+list[ele].id+'\',\''+list[ele].reqFlag+'\')"><i class="fa-table-line-submit" title="重发"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                // tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].manualAgvTaskId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
    ...
    $("#saveButton").on("click",function () {
            ...
            if(toAddressId==fromAddressId){
                $.DialogByZ.Autofade({Content: "起始地址与目的地址不能相同"});
                return false;
            }

            var data= {
                agvTaskType:agvTaskType,
            ...
            if (!_commonFun.isNotBlank(updateId)){
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/manualAgvTask/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
...
function editOneRecord(deviceId,type) {
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/manualAgvTask/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
...
function startOneRecord(id,reqFlag) {
    if (reqFlag=='1'){
        _commonFun.errorTips("该指令状态不可重发");
        return;
    }
    ...
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/callAgvLog/callAgvTask",
                headers:{'token':localStorage.getItem('token')},
                data    : {id:id},
                success	: function (res) {
                    var result=res;
                    if(result.code == 0){
                        _commonFun.successTips("重发成功");
                        _frontBasic.loadBasicList();
                    }else {
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/manualAgvTask/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {manualAgvTaskId:id},
                success	: function (res) {
                    var result=res;
                    if(result.code == 0){
                        _commonFun.successTips(result.data);
