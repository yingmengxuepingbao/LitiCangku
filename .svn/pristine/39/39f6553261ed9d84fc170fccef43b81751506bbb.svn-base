...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/supplier/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                supplierCode    : $.trim($('#supplierCodePoP').val()),
                supplierFullName          : $.trim($('#supplierFullNamePoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                tbody+='<tr data-id="'+list[ele].supplierId+'">' +
                    '<td>'+list[ele].supplierCode+'</td>'+
                    '<td>'+list[ele].supplierFullName+'</td>'+
                    '<td>'+list[ele].supplierShortName+'</td>'+
                    '<td>'+list[ele].supplierEngName+'</td>'+
                    '<td>'+list[ele].supplierType+'</td>'+
                    '<td>'+list[ele].supplierLevel+'</td>'+
                    '<td>'+list[ele].address+'</td>'+
                    '<td>'+list[ele].contacts+'</td>'+
                    '<td>'+list[ele].website+'</td>'+
                    '<td>'+list[ele].registrationDate+'</td>';
                tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].supplierId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].supplierId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
