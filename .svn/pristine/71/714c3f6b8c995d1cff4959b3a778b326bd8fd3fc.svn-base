...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/callFourwaycarLog/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                groupId    : $.trim($('#groupIdPop').val()),
                interfaceName : $.trim($('#interfaceNamePop').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].id+'">' +
                    '<td style="width: 5%">'+list[ele].id+'</td>'+
                    '<td style="width: 7%">'+list[ele].groupId+'</td>'+
                    '<td style="width: 7%">'+list[ele].interfaceName+'</td>'+
                    '<td style="width: 20%">'+list[ele].reqParam+'</td>'+
                    '<td style="width: 5%">'+reqFlagText+'</td>'+
                    '<td style="width: 20%">'+list[ele].respData+'</td>'+
...
function startOneRecord(id,reqFlag) {
    ...
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/callFourwaycarLog/resend",
                headers:{'token':localStorage.getItem('token')},
                data    : {id:id},
                success	: function (res) {
                    var result=res;
