...
$(function(){
    _box_service={
        ...
        getEdgeBoxs: function(edgeBoxInfo,resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/jobManagementService/list",
                headers:{'token':localStorage.getItem('token')},
                data	: edgeBoxInfo,
                success	: resolve,
                error	: reject
        ...
        loadEdgeBoxsHtml: function (list) {
            ...
            for (var ele in list) {
                str+='<tr id="'+list[ele].id+'">' +
                    ...
                    '<td class="jobStatus ">'+list[ele].jobStatus+'</td>'+
                    '<td class="errorMsg ">'+list[ele].errorMsg+'</td>'+
                    '<td class="elapsedTime">'+list[ele].elapsedTime+'</td>'+
