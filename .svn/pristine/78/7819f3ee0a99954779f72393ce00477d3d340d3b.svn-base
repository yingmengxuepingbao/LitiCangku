...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/platform/list",
        ...
        getDictType :function(receivers){
            ...
            $.ajax({
                type	:"get",
                url		: $.mpbGetHeaderPath()+"/dictItem/getDictType/platform_attr",
                headers:{'token':localStorage.getItem('token')},
                data    : { code :"area_attr"},
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            var info={
                ...
                platformCode    : $.trim($('#platformCodePoP').val()),
                platformName          : $.trim($('#platformNamePoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].platformId+'">' +
                    '<td>'+list[ele].warehouseCode+'</td>'+
                    '<td>'+list[ele].platformCode+'</td>'+
                    '<td>'+list[ele].platformName+'</td>'+
                    '<td>'+list[ele].sysAddress+'</td>';
                tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].platformId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].platformId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
    ...
    $("#saveButton").on("click",function () {
            ...
            var platformCode = $.trim($("#platformCode").val());
            var platformName = $.trim($("#platformName").val());
            var sysAddress = $.trim($("#sysAddress").val());

            //添加数据校验
            if(_commonFun.isBlank(platformCode)){
                $.DialogByZ.Autofade({Content: "月台编码不能为空"});
                return false;
            }
            if(_commonFun.isBlank(platformName)){
                $.DialogByZ.Autofade({Content: "月台名称不能为空"});
                return false;
            }
            ...
            //判断是新增还是修改
            if (!_commonFun.isNotBlank(platformId)){
                //不存在新增
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/platform/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/platform/u",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
...
function editOneRecord(platformId,type) {
    var id = platformId;
    $("#type").val(type);
    ...
    if(type == 0 ){
        ...
        $("#myModalLabel").text('查看月台信息');
        $("#saveButton").hide();
        $("form  select").prop("disabled",true);
        $("form  input").prop("disabled",true);
    }else if (type == 1){
        $("#formInfo").find("i").show();
        $("#myModalLabel").text('编辑月台信息');
        //编辑时，显示 提交按钮和启用下拉框和 input
        $("#saveButton").show().text("保存");
        $("form  select").prop("disabled",false);
        $("form  input").prop("disabled",false);
        //路线编码默认不可编辑
        $("#platformCode").prop("disabled",true);
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/platform/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
            if(result.code == 0){
                ...
                if(data != null){
                    ...
                    $("#platformCode").val(data.platformCode);
                    $("#platformName").val(data.platformName);
                    $("#sysAddress").val(data.sysAddress);
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/platform/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {platformId:id},
                success	: function (res) {
                    var result=res;
