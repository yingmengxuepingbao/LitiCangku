...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/wmsStoragePlane/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                locationCode : $.trim($('#locationCode').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                tbody+='<tr data-id="'+list[ele].storagePlaneId+'">' +
                    ...
                    '<td>'+list[ele].availableAmount+'</td>'+
                    '<td>'+list[ele].lockAmount+'</td>';
                tbody +=  '<td><a  onclick="editSelect(\''+list[ele].locationCode+','+ list[ele].goodsCode + ',' + list[ele].batchNo +'\')" > <i class="fa-table-line-look" title="查看"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>';
            }
    ...
    $("#exportExcel").on("click",function () {
        ...
        window.location.href=$.mpbGetHeaderPath()+"/excel/plane?locationCode="+locationCode
            +"&goodsCode="+goodsCode+"&batchNo="+batchNo;
    });
...
function editSelect(locationCode, goodsCode, batchNo) {
    window.location.href="wmsPalletInfoList.html?locationCode="+locationCode+"," + goodsCode + ","+batchNo;
}
