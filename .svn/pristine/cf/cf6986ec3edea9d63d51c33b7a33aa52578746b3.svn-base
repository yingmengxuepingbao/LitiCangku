...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/warehouse/list",
        ...
        //获取字典
        getDictType : function(data, reject){
            $.ajax({
                type	:"get",
                url		: $.mpbGetHeaderPath()+"/dictItem/getDictType/wh_type",
                headers:{'token':localStorage.getItem('token')},
                data    : { code :"wh_type"},
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            var info={
                ...
                warehouseName          : $.trim($('#warehouseNamePoP').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                warehouseCapacity =list[ele].warehouseCapacity == null?"":list[ele].warehouseCapacity;
                tbody+='<tr data-id="'+list[ele].warehouseId+'">' +
                    '<td>'+list[ele].warehouseCode+'</td>'+
                    '<td>'+list[ele].warehouseName+'</td>'+
                    '<td>'+list[ele].warehouseAddr+'</td>'+
                    '<td>'+list[ele].warehouseAttr+'</td>'+
                    '<td>'+warehouseCapacity+'</td>'+
                    '<td>'+list[ele].factoryName+'</td>'+
                    '<td>'+list[ele].address+'</td>';
                tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].warehouseId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].warehouseId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
    ...
    $("#saveButton").on("click",function () {
            ...
            var warehouseAddr = $.trim($("#warehouseAddr").val());
            var warehouseAttr = $.trim($("#warehouseAttr").val());
            var warehouseCapacity = $.trim($("#warehouseCapacity").val());
            var address = $.trim($("#address").val());
            var factoryName = $.trim($("#factoryName").val());
            //添加数据校验
            if(_commonFun.isBlank(warehouseCode)){
                $.DialogByZ.Autofade({Content: "仓库编码不能为空"});
                return false;
            }
            if(_commonFun.isBlank(warehouseName)){
                $.DialogByZ.Autofade({Content: "仓库名称不能为空"});
                return false;
            }
            ...
            //判断是新增还是修改
            if (!_commonFun.isNotBlank(warehouseId)){
                //不存在新增
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/warehouse/c",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
                ...
                $.ajax({
                    type	:"POST",
                    url		: $.mpbGetHeaderPath()+"/warehouse/u",
                    headers:{'token':localStorage.getItem('token')},
                    data    : data,
                    success	: function (res) {
                        var result=res;
...
function editOneRecord(warehouseId,type) {
    var id = warehouseId;
    $("#type").val(type);
    //供应商
    _buz_service.getDictType();
    ...
    if(type == 0 ){
        ...
        $("#myModalLabel").text('查看仓库信息');
        $("#saveButton").hide();
        $("form  select").prop("disabled",true);
        $("form  input").prop("disabled",true);
    }else if (type == 1){
        $("#formInfo").find("i").show();
        $("#myModalLabel").text('编辑仓库信息');
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/warehouse/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
            if(result.code == 0){
                ...
                if(data != null){
                    ...
                    $("#warehouseAddr").val(data.warehouseAddr);
                    $("#warehouseAttr").val(data.warehouseAttr);
                    $("#warehouseCapacity").val(data.warehouseCapacity);
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/warehouse/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {warehouseId:id},
                success	: function (res) {
                    var result=res;
