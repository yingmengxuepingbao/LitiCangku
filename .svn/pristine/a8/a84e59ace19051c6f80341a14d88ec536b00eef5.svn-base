...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/path/list",
        ...
        getSupplyList : function(data, reject){
            $.ajax({
                type: "post",
                url: $.mpbGetHeaderPath() + "/address/findaddress",
                headers: {'token': localStorage.getItem('token')},
                data: {
                },
                async: false,
                success: function (res) {
                    ...
                    if (result.code == 0) {
                        ...
                        $("#fromAddressId").html(options);
                        $("#toAddressId").html(options);
                    }
                },
                error: reject
            })
                ...
                success: function(result){
                    ...
                    switch(code){
                        case '0':
                            ...
                            $('#channelAddress').html(options);
                            $('#channelAddress').chosen({"width":280}).trigger("chosen:updated");
                            break;
                        default:
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            var _this = this;
            var info={
                pathId    : $.trim($('#pathIdPoP').val()),
                description          : $.trim($('#descriptionPop').val()),
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].pathId+'">' +
                    '<td>'+list[ele].areaCode+'</td>'+
                    '<td>'+list[ele].pathId+'</td>'+
                    '<td>'+list[ele].description+'</td>'+
                    '<td>'+list[ele].fromAddressId+'</td>'+
                    '<td>'+list[ele].toAddressId+'</td>'+
                    '<td>'+list[ele].channelAddress+'</td>';
                tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].pathId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].pathId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
    ...
    $("#saveButton").on("click",function () {
            ...
            var fromAddressId = $.trim($("#fromAddressId").val());
            var toAddressId = $.trim($("#toAddressId").val());
            $("#channelAddress option:selected").each(function () {
                channelIds.push($(this).attr('value'));
            });
            var channelAddress	=channelIds.toString();
            var description = $.trim($("#description").val());
            var areaCode = $.trim($("#areaCode").val());


            //添加数据校验
            if(_commonFun.isBlank(pathId)){
                $.DialogByZ.Autofade({Content: "路径ID不能为空"});
                return false;
            }
            if(_commonFun.isBlank(description)){
                $.DialogByZ.Autofade({Content: "路径描述不能为空"});
                return false;
            }

            if(_commonFun.isBlank(fromAddressId)){
                $.DialogByZ.Autofade({Content: "起始地址不能为空"});
                return false;
            }
            if(_commonFun.isBlank(toAddressId)){
                $.DialogByZ.Autofade({Content: "目的地址不能为空"});
                return false;
            }
            // if(_commonFun.isBlank(channelAddress)){
            //     $.DialogByZ.Autofade({Content: "途径地址不能为空"});
            //     return false;
            // }
            console.log("channelIds",channelIds);
            //途径路径判断
            if(fromAddressId==toAddressId){
                $.DialogByZ.Autofade({Content: "起始地址不能等于目的地址"});
                return false;
            }
            if (!_commonFun.isBlank(channelAddress)) {
                if(channelIds.indexOf(fromAddressId)!=-1){
                    $.DialogByZ.Autofade({Content: "途径地址不能包含起始地址"});
                    return false;
                }
                if(channelIds.indexOf(toAddressId)!=-1){
                    $.DialogByZ.Autofade({Content: "途径地址不能包含目的地址"});
                    return false;
                }
            }
...
function editOneRecord(pathId,type) {
    var id = pathId;
    $("#type").val(type);
    //供应商
    _buz_service.getSupplyList();
    ...
    if(type == 0 ){
        ...
        $("#myModalLabel").text('查看路径信息');
        $("#saveButton").hide();
        $("form  select").prop("disabled",true);
        $("form  input").prop("disabled",true);
    }else if (type == 1){
        $("#formInfo").find("i").show();
        $("#myModalLabel").text('编辑路径信息');
        //编辑时，显示 提交按钮和启用下拉框和 input
        $("#saveButton").show().text("保存");
        $("form  select").prop("disabled",false);
        $("form  input").prop("disabled",false);
        //路线编码默认不可编辑
        $("#pathId").prop("disabled",true);
    ...
    $.ajax({
        type	:"get",
        url		: $.mpbGetHeaderPath()+"/path/r/"+id,
        headers:{'token':localStorage.getItem('token')},
        data    : {id:id},
        success	: function (res) {
            var result=res;
            if(result.code == 0){
                var data = result.data;
                if(data != null){
                    $("#updateId").val(data.pathId);
                    $("#pathId").val(data.pathId);

                    $("#fromAddressId").val(data.fromAddressId);
                    $("#toAddressId").val(data.toAddressId);

                    _buz_service.getAddress(data.channelAddress);

                    $("#channelAddress").val(data.channelAddress);
...
function deleteOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/path/delete",
                headers:{'token':localStorage.getItem('token')},
                data    : {pathId:id},
                success	: function (res) {
                    var result=res;
