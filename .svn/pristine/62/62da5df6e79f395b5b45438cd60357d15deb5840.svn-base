    ...
    var _person= {
        ...
        bindEvent: function () {
            ...
            $(document).on('click', '#addEmployeeBtn', function () {
                ...
                $(".cusPopover-title span").text("新增员工");
                $(".errorMsg").empty();
                ...
                $("#allRoles").hide();
                //区分是搜索的部门还是弹窗里的部门
                $("#employeeCodePop").removeAttr("disabled");
            });
            ...
            $(document).on('click', '#personList-table .a-peron-edit', function (e) {
                ...
                var _self = $(this);
                selectPopTreeNode.cancelSelectedNode();
                $("#employeeCodePop").attr("disabled","disabled");
                $(".fa-personInfoPop").hide();
                var employeeId = _self.parent().parent().data('employeeid');
                $("#hideEmployeeIdPop").val(employeeId);
                //所有角色清空勾选
                // $("#allRoles .checkbox-custom").find('input[type="checkbox"]').prop("checked",false);
                $("#personPopover").show();
                $("#allRoles").show();
                new doAjax({
                    ...
                    callBack: function (result) {
                        if(result.code==0){
                            ...
                            $("#employeeCodePop").val(data.account);
                            $("#employeeNamePop").val(data.nickname);
            ...
            $(document).on('click', '#savePersonBtn', function () {
                ...
                if(employeeCode==""){
                    $("#employeeCodePop").css("border","1px solid #EF4836");
                    $(".fa-personInfoPop").show();
                    $(".fa-personInfoPop").text("员工编码，不能为空！");
                    return;
                }
                $("#employeeCodePop").removeAttr('style');
                if(employeeName==""){
                ...
                $(".fa-personInfoPop").hide();
                var employeeId=$.trim($("#hideEmployeeIdPop").val());
                if(employeeId==""){
                    //新增 保存员工
                    var data={
                    ...
                    new doAjax({
                        ...
                        callBack: function (result) {
                            if(result.code==0) {
                                $.DialogByZ.Autofade({Content: "新增员工成功！"});
                                $("#personPopover").hide();
                                //刷新员工列表
                                _this.loadEmployeeList();
                            }
                    ...
                    new doAjax({
                        ...
                        callBack: function (result) {
                            if(result.code==0) {
                                $.DialogByZ.Autofade({Content: "修改员工成功！"});
                                $("#personPopover").hide();
                                //刷新员工列表
                                _this.loadEmployeeList();
                            }
        ...
        loadEmployeeList : function () {
            ...
            var keyInfo={
                account:        '',
                nickname        :$.trim($("#employeeNameS").val()),
        ...
        loadEmployeeSource : function(list) {
            ...
            for(ele in list){
                ...
                str+='<tr data-employeeid="'+list[ele].id+'"><td class="employeeCode">'+list[ele].account+'</td>'+
                    '<td class="employeeName">'+list[ele].nickname+'</td>'+
                    '<td class="departmentName">'+departmentName+'</td>'+
                    '<td class="postName">'+postName+'</td>'+
                    ...
                    '<a class="link a-resetPassword" >重置密码</a>&nbsp;&nbsp;'+statusStr+
                ...
                callBack: function (result) {
                    if(result.code==0){
                        ...
                        for(var j=0;j<list.length;j++){
                            options+="<option value='"+list[j].id+"'>"+list[j].description+"</option>";
                        }
            ...
            if(rs && rs.length>0){
                ...
                for(var i=0;i<rs.length;i++){
                    roleStr+='<div class="checkbox-custom">' +
                        '<input type="checkbox" value="'+rs[i].id+'" class="customCk" name="checkbox">' +
        ...
        showPostList:function (ps) {
            ...
            if(ps && ps.length>0){
                ...
                for(var i=0;i<ps.length;i++){
                    ...
                    '<input type="checkbox" value="'+ps[i].id+'" class="customCk" name="checkbox">' +
