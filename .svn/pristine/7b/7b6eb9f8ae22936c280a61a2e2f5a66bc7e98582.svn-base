...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/fourwaycarReportLog/list",
    ...
    _frontBasic = {
        ...
        loadBasicList : function () {
            ...
            var receiveTimeMin = $.trim($('#receiveTime_begin').val());
            if(receiveTimeMin!=""&&receiveTimeMin!=null){
                receiveTimeMin = $.trim($('#receiveTime_begin').val()) + " 00:00:00";
            }
            var receiveTimeMax = $.trim($('#receiveTime_end').val());
            if(receiveTimeMax!=""&&receiveTimeMax!=null){
                receiveTimeMax = $.trim($('#receiveTime_end').val()) + " 23:59:59";
            }
            var info={
                taskId    : $.trim($('#taskIdPop').val()),
                reportType    : $.trim($('#reportTypePop').val()),
                returnStatus : $.trim($('#returnStatusPop').val()),
                receiveTimeMin  : receiveTimeMin ,
                receiveTimeMax  : receiveTimeMax ,
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                var returnStatusText ="";
                var returnStatus = list[ele].returnStatus;
                if(returnStatus =="0"){
                    returnStatusText = "成功";
                ...
                tbody+='<tr data-id="'+list[ele].id+'">' +
                    ...
                    '<td>'+list[ele].reportType+'</td>'+
                    '<td>'+list[ele].inParam+'</td>'+
                    '<td>'+returnStatusText+'</td>'+
                    '<td>'+list[ele].returnInfo+'</td>'+
                    '<td>'+list[ele].receiveTime+'</td>'+
                    '<td>'+list[ele].respTime+'</td>';
                tbody += '</tr>'
            }
