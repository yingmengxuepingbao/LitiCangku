...
$(function(){
    _box_service={
        ...
        getEdgeBoxs: function(edgeBoxInfo,resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/wcsJobExecutionSummaryService/list",
                headers:{'token':localStorage.getItem('token')},
                data	: edgeBoxInfo,
                success	: resolve,
                error	: reject
...
//-----------------主体_edgeBox-begin----------------------
    _edgeBox= {
        ...
        bindEvent: function () {
            ...
            //搜索文本框-作业名称
            $("#jobName_search").bind("input propertychange", function () {
                $(".currentPage").text("1");
                $("#check-all").prop("checked", false);
                _this.loadEdgeBoxs();
            });
        ...
        loadEdgeBoxs: function () {
            ...
            var _this      =this;
            var equipment  ={
                jobName    : $.trim($("#jobName_search").val()),
        ...
        loadEdgeBoxsHtml: function (list) {
            ...
            var str = '';
            for (var ele in list) {
                str+='<tr id="'+list[ele].errId+'" palletCode="'+list[ele].palletCode+'"goodsName="'+list[ele].goodsName+'"batchNo="'+list[ele].batchNo+'"moveNo="'+list[ele].moveNo+'"fromLocationCode="'+list[ele].fromLocationCode+'"toLocationCode="'+list[ele].toLocationCode+'">' +
                    // '<td><div class="checkbox-custom checkbox-default"><input type="checkbox" >' +
                    // '<label></label></div></td>'+
                    '<td class="jobName">'+list[ele].jobName+'</td>'+
                    '<td class="summaryHour ">'+list[ele].summaryHour+'</td>'+
                    '<td class="jobInterval ">'+list[ele].jobInterval+'</td>'+
                    '<td class="totalCount">'+list[ele].totalCount+'</td>'+
                    '<td class="errorCount ">'+list[ele].errorCount+'</td>'+
                    '<td class="totalElapsedTime">'+list[ele].totalElapsedTime+'</td>'+
                    '<td class="avgElapsedTime">'+list[ele].avgElapsedTime+'</td>'+
                    '<td class="maxElapsedTime">'+list[ele].maxElapsedTime+'</td>'+
                    '<td class="minElapsedTime">'+list[ele].minElapsedTime+'</td>';
