...
$(function(){
    _box_service={
        ...
        getEdgeBoxs: function(edgeBoxInfo,resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/wmsOrderOutStereoscopic/list",
    ...
    _edgeBox= {
        ...
        bindEvent: function () {
            ...
            $(document).on('click', '#addEquipment', function () {
                ...
                window.location.href="addNHMoveStereoscopic.html";
            });
            //点击EdgeBox采集终端列表中的编辑
            $(document).on('click', '.a-equipment-edit', function (e) {
                ...
                alert("id"+ id)
                window.location.href="addNHMoveStereoscopic.html?id="+id;
            })
            //全选/全不选 checkbox
...
*/
            ...
            $(document).on('click', '.taskStartList', function () {
                ...
                _box_service.taskStartListBoxInfo(monitorStr, function (res) {
                    ...
                    if(result.code==0) {
                        //已生成，调任务列表页面
                        window.location.href="wmsNHCheckTaskLogList.html?orderNo="+monitorStr+"&taskType=22";
                    }else {
                        // 生成失败
        ...
        loadEdgeBoxsHtml: function (list) {
            ...
            for (var ele in list) {
                ...
                if(list[ele].orderStatus == 2){
                    ...
                    str += '<a onclick="disableOneRecord(\''+list[ele].orderNo+'\')"> <i class="fa-table-line-para"   title="停用"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                    // }
                }
                if(list[ele].orderStatus == 1 ){
...
function editSelect(orderNo) {
    window.location.href="addNHMoveStereoscopic.html?orderNo="+orderNo;
}
...
function editOneRecord(orderNo,type) {
    // alert(orderNo);
    if(type == 0){
        window.location.href="infoOrderOutStereoscopic.html?orderNo="+orderNo+','+type;
    }else{
        window.location.href="addNHMoveStereoscopic.html?orderNo="+orderNo+','+type;
    }
}
//停用edgeBox
