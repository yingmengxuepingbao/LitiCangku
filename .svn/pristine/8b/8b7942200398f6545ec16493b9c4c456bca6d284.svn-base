...
function menuEvent(container,targeta,target){
    ...
    if(targeta.next('ul')[0]){
        ...
        var menuType = targeta.attr('menuType');
        if(menuurl == ' '){
            return;
        }
        ...
        if(opentaglist[menuid]){
            ...
            //如果不存在，创建并打开
            createTab(menuid,menuname,menuurl,menuType)
        }
...
function createTab(menuid,tabName,url,type){
    var tabmenu= $(".tabmenu .centercontainer ul");
    var tagspan=$('<div><span menuid="'+menuid+'">'+tabName+'</span><span class="iconfont icon-f14" onclick="reload(event)"></span><span class="iconfont icon-guanbi" onclick="destorytagandiframe(event)"></span></div>')
    ...
    selectTab(menuid);
    createNewIframe(menuid,url,type);
    //判断是否点亮左右移动标签
    var centercontainer=parseInt($('.tabmenu .centercontainer').css('width'));
    var ullength=parseInt($('.centercontainer ul').css('width'));
    var leftarrow=$('#maincontainer .tabmenu .leftarrow');
...
//创建窗口   传入窗口id和url
function createNewIframe(iframeid,url,type){
    ...
    if(type=="3"){
        iframe.src =$.mpbGetHeaderPath()+url;
    }
    else{
        iframe.src = url;
    }
...
accordionMenu.prototype = {
    ...
    _renderHTML: function(container){
        ...
        $(_config.menujson).each(function(index,item){
            ...
            var lihtml = $('<li><p class="spancon pclose" menuurl="'+url+'" menuid="'+id+'" menuname="'+name+'" menuType="'+resType+'"><span class="textContainer"><span class="menuname">'+name+'</span></span></p></li>');
            ...
            var img=$('<img src="'+$.mpbGetHeaderPath()+item.icon+'"\>');
...
// 当index页左侧菜单是min模式 ，则添加菜单点击即收起的事件
$(document).on("click",".menu-ms #menu .level-1 li",function (e) {
    var _self=$(this);
    if(!_self.find('ul').hasClass("level-2")){
        _self.parents(".level-1").hide();
    }
    e.stopPropagation();
})