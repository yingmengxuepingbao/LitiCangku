...
$(function(){
    _buz_service={
        ...
        getDataList : function(data, resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/manualAgvTask/list",
    ...
    _frontBasic = {
        ...
        basicListSource : function(list) {
            ...
            for(var ele in list){
                ...
                tbody+='<tr data-id="'+list[ele].manualAgvTaskId+'">' +
                    '<td>'+list[ele].taskNo+'</td>'+
                    '<td>'+list[ele].agvTaskType+'</td>'+
                    '<td>'+taskStatusText+'</td>'+
                    '<td>'+list[ele].fromAddressId+'</td>'+
                    '<td>'+list[ele].fromAddressName+'</td>'+
                    '<td>'+list[ele].toAddressId+'</td>'+
                    '<td>'+list[ele].toAddressName+'</td>'+
                    '<td>'+list[ele].createBy+'</td>'+
                    '<td>'+list[ele].gmtCreate+'</td>';
                tbody += '<td>'+'<a onclick="startOneRecord(\''+list[ele].manualAgvTaskId+'\',1)"><i class="fa-table-line-submit" title="下发任务"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                // tbody += '<td>'+'<a onclick="editOneRecord(\''+list[ele].manualAgvTaskId+'\',1)"><i class="fa-table-line-edit" title="编辑"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody +=  '<a onclick="deleteOneRecord(\''+list[ele].manualAgvTaskId+'\')"><i class="fa-table-line-delete" title="删除"></i></a>&nbsp;&nbsp;&nbsp;&nbsp;';
                tbody += '</td>'+'</tr>'
            }
...
function startOneRecord(id) {
    $.DialogByZ.Confirm({
        ...
        FunL:function(){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/manualAgvTask/startTask",
                headers:{'token':localStorage.getItem('token')},
                data    : {manualAgvTaskId:id},
                success	: function (res) {
                    var result=res;
                    if(result.code == 0){
                        _commonFun.successTips("下发成功");
                        _frontBasic.loadBasicList();
                    }else {
