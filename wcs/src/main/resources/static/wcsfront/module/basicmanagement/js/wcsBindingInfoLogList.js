...
$(function(){
    _box_service={
        ...
        getEdgeBoxs: function(edgeBoxInfo,resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/bindingInfo/list",
                headers:{'token':localStorage.getItem('token')},
                data	: edgeBoxInfo,
                success	: resolve,
                error	: reject
        ...
        loadEdgeBoxsHtml: function (list) {
            ...
            for (var ele in list) {
                str+='<tr id="'+list[ele].id+'">' +
                    ...
                    // '<label></label></div></td>'+
                    // '<td class="palletizerId">'+list[ele].palletizerId+'</td>'+
                    '<td class="addressId">'+list[ele].addressId+'</td>'+
                    ...
                    '<td class="jobName">'+list[ele].reportWms+'</td>'+
                    // '<td class="recommendedLocation ">'+list[ele].recommendedLocation+'</td>'+
                    // '<td class="isCross ">'+list[ele].isCross+'</td>'+
                    '<td class="errorMsg" style="word-wrap:break-word;width: 20%">'+list[ele].errorMsg+'</td>'+
