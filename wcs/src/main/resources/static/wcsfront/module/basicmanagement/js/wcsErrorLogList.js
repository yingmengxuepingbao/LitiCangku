...
$(function(){
    _box_service={
        ...
        getEdgeBoxs: function(edgeBoxInfo,resolve, reject){
            $.ajax({
                type	:"post",
                url		: $.mpbGetHeaderPath()+"/wcsErrorLogService/list",
                headers:{'token':localStorage.getItem('token')},
                data	: edgeBoxInfo,
                success	: resolve,
                error	: reject
...
//-----------------主体_edgeBox-begin----------------------
    _edgeBox= {
        ...
        bindEvent: function () {
            ...
            //搜索文本框-设备编码
            // $("#deviceId_search").bind("input propertychange", function () {
            //     $(".currentPage").text("1");
            //     $("#check-all").prop("checked", false);
            //     _this.loadEdgeBoxs();
            // });
            //搜索文本框-设备类型
            $("#deviceTypeSelect").bind("input propertychange", function () {
                $(".currentPage").text("1");
                $("#check-all").prop("checked", false);
                _this.loadEdgeBoxs();
            });
            //搜索文本框-故障类型
            $("#errorTypeSelect").bind("input propertychange", function () {
                $(".currentPage").text("1");
                $("#check-all").prop("checked", false);
                _this.loadEdgeBoxs();
            });
            //搜索文本框-故障码
            // $("#errCode_search").bind("input propertychange", function () {
            //     $(".currentPage").text("1");
            //     $("#check-all").prop("checked", false);
            //     _this.loadEdgeBoxs();
            // });
            //搜索文本框-故障等级
            $("#levelSelect").bind("input propertychange", function () {
                $(".currentPage").text("1");
                $("#check-all").prop("checked", false);
                _this.loadEdgeBoxs();
            });
        ...
        loadEdgeBoxs: function () {
            ...
            var _this      =this;
            var equipment  ={
                deviceId    : $.trim($("#deviceId_search").val()),
                deviceType    : $.trim($("#deviceTypeSelect").val()),
                errType    : $.trim($("#errorTypeSelect").val()),
                errCode    : $.trim($("#errCode_search").val()),
                level          : $.trim($("#levelSelect").val()),
        ...
        loadEdgeBoxsHtml: function (list) {
            ...
            for (var ele in list) {
                str+='<tr id="'+list[ele].moveId+'" palletCode="'+list[ele].palletCode+'"goodsName="'+list[ele].goodsName+'"batchNo="'+list[ele].batchNo+'"moveNo="'+list[ele].moveNo+'"fromLocationCode="'+list[ele].fromLocationCode+'"toLocationCode="'+list[ele].toLocationCode+'">' +
                    ...
                    // '<label></label></div></td>'+
                    '<td class="deviceId" style="width: 12%">'+list[ele].deviceId+'</td>'+
                    '<td class="deviceType" style="width: 12%">'+list[ele].deviceType+'</td>'+
                    '<td class="errType" style="width: 12%">'+list[ele].errType+'</td>'+
                    '<td class="errCode" style="width: 10%">'+list[ele].errCode+'</td>'+
                    '<td class="levelStr" style="width: 12%">'+list[ele].levelStr+'</td>'+
                    '<td class="faultSource" style="width: 10%">'+list[ele].faultSource+'</td>'+
                    '<td  style="width: 10%;word-wrap:break-word;">'+list[ele].instruction+'</td>'+
                    '<td class="description" style="width: 10%">'+list[ele].description+'</td>'+
                    '<td class="gmtCreate" style="width: 12%">'+list[ele].gmtCreate+'</td>';
